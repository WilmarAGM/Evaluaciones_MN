...
Spline cúbico extrapolado

Este método encuentra el spline cúbico extrapolado que se ajusta a un conjunto de datos,
...
def SplineExtra(X, Y) -> np.array:
     ...
     a = h[1:n-1].copy()
     b[0] = b[0] + h[0] + h[0]*h[0]/h[1]     
     c[0] = c[0] -h[0]*h[0]/h[1]
     b[n-2] = b[n-2] + h[n-1] + h[n-1]*h[n-1]/h[n-2]
     a[n-3] = a[n-3] - h[n-1]*h[n-1]/h[n-2]
     ...
     M[0] = M[1] - h[0]*M[2]/h[1]+h[0]*M[1]/h[1]
     M[-1] = M[n-1] + h[n-1]*(M[n-1]-M[n-2])/h[n-2]
