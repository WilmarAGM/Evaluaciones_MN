...
export default function ChangePasswordForm() {
  ...
  // Se pinta en <body> con un portal: los encabezados de las páginas usan
  // backdrop-blur, y eso hace que un `fixed` dentro de ellos se posicione
  // respecto al encabezado (no a la pantalla) y el diálogo se vea cortado.
  return (
    <>
      {trigger}
      {createPortal(
        ...
        >
          <form
            ...
            className="w-full max-w-sm bg-[#12142a] border border-white/10 rounded-2xl p-6 shadow-2xl my-auto"
          >
            ...
            <div className="flex gap-3 mt-4">
              <button
                type="button"
                onClick={close}
                className="flex-1 rounded-lg border border-white/10 text-slate-300 hover:text-white hover:bg-white/5 transition py-2.5 font-medium"
              >
                {ok ? "Cerrar" : "Cancelar"}
              </button>
              {!ok && (
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 rounded-lg bg-gradient-to-r from-brand-600 to-fuchsia-600 hover:from-brand-500 hover:to-fuchsia-500 transition text-white font-semibold py-2.5 disabled:opacity-60"
                >
