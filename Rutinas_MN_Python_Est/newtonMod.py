...
El método de Newton-Raphson modificado para encontrar una raíz de la función f.

Parámetros:
- f (función): La función cuya raíz se busca.
- df (función): La primera derivada de la función f.
- d2f (función): La segunda derivada de la función f.
...
Retorna:
- p_next(float): Una aproximación de la raíz.
- None: Si el método no converge dentro de max_iter iteraciones.

Excepciones:
- ZeroDivisionError: Si el denominador es cero en alguna iteración.
"""
...
def newton_mod(f, df, d2f, p0, tol, max_iter):
    ...
    for n in range(1, max_iter + 1):
        ...
        if (df_p)**2 -f_p*d2f_p == 0:
            raise ZeroDivisionError(f"El denominador es cero en la iteración {n}. No se encontró solución.")
        
        # Calcular la siguiente aproximación
        p_next = p - f_p * df_p/ (df_p**2 - f_p*d2f_p)
